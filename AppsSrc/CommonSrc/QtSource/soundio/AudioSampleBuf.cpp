...
int AudioSampleBuf::writeSamples( int16_t* srcSamplesBuf, int sampleCnt )
{
	...
	{
		LogMsg( LOG_ERROR, "AudioSampleBuf::writeSamples ERROR samplesToWrite %d greater than max samples %d", sampleCnt, m_MaxSamples );
		return 0;
	}
	...
	else
	{
		...
		LogMsg( LOG_WARNING, "AudioSampleBuf::writeSamples removing %d samples to fit %d samples", samplesToRemove, sampleCnt );
